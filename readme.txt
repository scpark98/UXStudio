[수정될 내용]
- 어떤 항목의 좌표정보에 해당 항목이 속해있는 parent 항목과의 거리도 표시해야 한다.
  UI 상에서 해당 항목의 parent를 클릭 지정 또는 parent index 지정이 가능해야 한다.
  이 후 index 변경시에는 관련된 항목들 정보도 모두 갱신해줘야 한다.
- 순서 변경
- 방향키로 위치, 크기 변경
- canvas color or image
- 순서 정렬 기능 : left -> top 순서 정렬기능이 있어야 이웃한 항목 순서로 출력할 수 있다.

[이미지로 저장하기 위한 방식]
- device context를 canvas 크기대로 생성하지 않고 현재 view 크기로 생성하고
  on_resize시에도 view 크기대로만 조정해주면 화면엔 잘 표시되나
  이미지로 저장할 경우 보이는 크기대로만 저장되는 문제가 있다.
  심지어는 device context 생성 시 기본 bitmap 변수를 멤버변수로 선언하면
  UI가 정상적으로 동작하지 않는 부작용이 생긴다.

- device context 크기를 canvas 크기로 고정시키면 이미지로 잘 저장되지만
  화면에 전혀 표시되지 않는 문제가 있다.

[zoom 기능 구현]
- OnDraw()에서 SetTransform()을 이용하면 확대/축소해서 그리는 것은 쉽지만
  스크롤 범위, 마우스 액션 등은 모두 보정한 후 처리해줘야 한다.
  해당되는 모든 수치는 int가 아닌 float이어야 한다.

[이동 및 크기조정 시 Ctrl, Shift, Alt 키 조합 규칙]
- ppt의 규칙을 기본으로 한다.
- 방향키만 누르면 grid 만큼 씩 이동,
  shift를 누르면 1씩 이동,
  ctrl을 누르면 grid * 2씩 이동,
  shift+ctrl이면 grid * 4씩 이동한다.
  마우스로 이동 시 alt키를 누르면 다른 항목과의 align을 무시한다.


[수정된 내용]

[정렬]
- 어떤 항목이 다른 항목의 안에 존재하면 그 항목을 앞으로 보낸다.
  포함관계가 없다면 left 순서로, left < m 이하라면 y <= n 이면 순서대로

- 포함 관계가 없다면 왼쪽에 있는 항목이 우선한다.
  단, y <= n인 경우에만 적용되며 그 이상인 경우는 같은 그룹 항목이 아니다.

[인덱스 정렬 규칙]
- 직관적으로 큰 크기의 항목 안에 작은 항목들이 존재해야 한다.
  따라서 큰 항목의 인덱스가 더 우선되어야 한다. (큰 항목 index < 그 내부 항목들의 인덱스)
  이래야만 직관적으로 이해되고 좌표값 출력시에도 순서대로 출력된다.
- 새 항목이 추가되면 리스트의 맨 뒤에 추가된다.
- 그릴때는 맨 처음 항목부터 그려진다.

[멀티선택 구현방식]
1. 각 el의 is_selected flag를 저장하는 방법 (figma 방식)
- 선택항목은 선택되었음을 그려준다.
- 선택정보 또한 저장할 수 있다. vector 방식도 선택여부를 저장할 수 있다.(save시에 flag 세팅)
- m_r_selected를 구하고 이는 최대 사각형을 의미한다.
- move, resize시에 모든 루프를 돌면서 선택된 항목에 대해서만 처리하므로 매번 모든 루프를 돌아야 한다.

2. std::vector<CSCUIElement*> m_item_selected;
- 선택된 항목들을 vector로 저장한다. (ppt 방식)
- 선택 항목은 선택되었음을 그려준다.
- m_r_selected? 를 구하고 이는 최대 사각형을 의미한다.

- OnMouseMove()에서는 m_r_selected안에 있으면 반응하지 않는다.

[자석기능]
- move/resize시에 다른 항목과의 자동 정렬을 지원해야 한다.
- l, t, r, b, cp, canvas_cp 등 어디에 일치하는지를 기억해야 한다.
- a라는 항목의 l, t에 일치한다면 이를 기억해서 OnDraw()에서 그려줘야 한다.

[undo/redo]
- m_data의 복사본을 vector에 저장해서 불러오는 방식으로 구현

[속성창]
- label
- x y w h
- round (indivisual 클릭 시 각각 조절 가능)
- stroke color, alpha, width, style
- fill color, alpha, pattern

[스크롤 보정]
- 도형을 그릴때도 스크롤 보정을 하고 그려야 한다. 그래야만 grid를 따라 그릴 수 있다.
- 그려진 후에도 스크롤 보정된 좌표를 vector에 추가한다.
- 

[향후 수정될 내용]
- 편집 내용이 바로 서버에 저장되고 공유한 사용자 화면에 바로 적용해야 한다. (broadcast)
- 버전 관리 (repository, branch)